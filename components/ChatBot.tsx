...
const ChatBot: React.FC = () => {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      // Agregar placeholder vacío para streaming
      setMessages((prev) => [
        ...prev,
        {
          id: modelMsgId,
      ...
      for await (const chunk of stream) {
        // Extraer texto del chunk - text es una propiedad getter, no una función
        let chunkText = "";

        // Intentar obtener texto del chunk (es un getter, acceder directamente)
        try {
          if (chunk?.text) {
          ...
          // Si el getter de text falla, intentar con candidates
      ...
      setMessages((prev) => [
        ...prev,
        {
          id: Date.now().toString(),
          role: "model",
          text: "Perdón, tuve un problema de conexión. ¿Podés repetir?",
          timestamp: new Date(),
        },
      ]);
    } finally {
            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <span className="text-xs text-blue-200 flex items-center gap-1">
                  <span className="w-2 h-2 bg-green-400 rounded-full"></span> En
                  línea
                </span>
              </div>
            </div>
